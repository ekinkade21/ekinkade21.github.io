...
const socials: Social[] = [
  {
    name: "GitHub",
    url: "https://github.com/ekinkade21",
  ...
  {
    name: "LinkedIn",
    url: "https://www.linkedin.com/in/elijah-kinkade-ek21/",
    icon: `<svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
...
function createExperienceCard(exp: Experience, index: number): string {
  return `
...
function createSocialLink(social: Social, index: number): string {
  return `
    <a href="${social.url}" 
       target="_blank" 
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (socialLinks) {
    socialLinks.innerHTML = socials.map((social, index) => 
      createSocialLink(social, index)
    ).join('');
  }
});